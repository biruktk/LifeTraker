...
interface SimpleChatProps {
  ...
  onUpdateMessages: (msgs: SavedMessage[]) => void;
}

const SimpleChat: React.FC<SimpleChatProps> = ({ userId, messages = [], onUpdateMessages }) => {
  const [input, setInput] = useState('');
  const [attachedImage, setAttachedImage] = useState<string | null>(null); // This is now a Preview URL
            ...
            {messages.map((msg) => (
                <div key={msg.id} className="group flex justify-end">
                    <div className="max-w-[85%] min-w-[200px] bg-black/40 border border-emerald-500/20 rounded-2xl rounded-tr-sm p-3 relative hover:bg-black/50 transition-colors">
                        ...
                        {msg.text && <div className="text-emerald-50 whitespace-pre-wrap leading-relaxed text-sm">{msg.text}</div>}
                    </div>
                </div>
            ))}
        ...
        <div className="p-4 bg-emerald-950/80 border-t border-emerald-500/20 backdrop-blur-md">
            {attachedImage && (
                <div className="mb-2 relative inline-block group">
                    ...
                    <button onClick={() => { setAttachedImage(null); setFileToUpload(null); }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs shadow-md hover:bg-red-600 z-10">×</button>
                </div>
            )}
            <div className="flex gap-2 items-end">
                <button onClick={toggleListening} className={`p-3 rounded-xl border border-emerald-500/30 transition-all ${isListening ? 'bg-red-500 text-white animate-pulse' : 'bg-black/30 text-emerald-400 hover:text-white hover:bg-emerald-800/50'}`}>🎙️</button>
                <button onClick={() => fileInputRef.current?.click()} className="p-3 rounded-xl bg-black/30 border border-emerald-500/30 text-emerald-400 hover:text-white hover:bg-emerald-800/50 transition-colors">
                ...
                <textarea 
                    ...
                    rows={1}
                />
                <button onClick={sendMessage} disabled={(!input.trim() && !fileToUpload) || isUploading} className="p-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-600/20 disabled:opacity-50 transition-all font-bold text-sm">
                    {isUploading ? '...' : 'SAVE'}
                </button>
            </div>
        </div>
