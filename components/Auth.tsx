...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const users = JSON.parse(localStorage.getItem('life_tracker_users') || '[]');
    ...
    if (isLogin) {
      ...
      localStorage.setItem('life_tracker_users', JSON.stringify([...users, newUser]));
      onLogin(newUser);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-emerald-950 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-emerald-900 to-emerald-950">
      <div className="glass-card w-full max-w-md p-8 rounded-3xl border border-emerald-500/20 shadow-2xl animate-fade-in">
